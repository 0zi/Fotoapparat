...
class TextureRendererView extends FrameLayout implements CameraRenderer {
  ...
  @Override public void attachCamera(CameraDevice camera) {
    awaitSurfaceTexture();
    updateLayout(camera);
  ...
  private void updateLayout(final CameraDevice camera) {
    final Size previewSize = toPreviewSize(camera.getRendererParameters());
  ...
  private Size toPreviewSize(RendererParameters rendererParameters) {
    return rendererParameters.frameRotation == 0 || rendererParameters.frameRotation == 180
        ? rendererParameters.previewSize : rendererParameters.previewSize.flip();
  }
  ...
  private void centerInside(Size previewSize) {
    final float scale = Math.min(getMeasuredWidth() / (float) previewSize.width,
    ...
    getChildAt(0).layout(extraX / 2, extraY / 2, width + (extraX / 2), height + (extraY / 2));
  }

  private void centerCrop(Size previewSize) {
    final float scale = Math.max(getMeasuredWidth() / (float) previewSize.width,
        getMeasuredHeight() / (float) previewSize.height);
