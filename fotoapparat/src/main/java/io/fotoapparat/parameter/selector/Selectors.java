...
public class Selectors {
   ...
   */
  @SafeVarargs public static <T> SelectorFunction<T> firstAvailable(
      @NonNull final SelectorFunction<T> function, final SelectorFunction<T>... functions) {
    return new SelectorFunction<T>() {

      @SuppressWarnings("unchecked") @Override public T select(Collection<T> items) {
  ...
  public static <T> SelectorFunction<T> filtered(@NonNull final SelectorFunction<T> selector,
    ...
    return new SelectorFunction<T>() {
      @Override public T select(Collection<T> items) {
        return selector.select(filteredItems(items, predicate));
      }
    };
  }
  ...
  public static <T> SelectorFunction<T> single(final T preference) {
    return new SelectorFunction<T>() {
      @Override public T select(Collection<T> items) {
        return items.contains(preference) ? preference : null;
      }
    };
  }
