...
public class BitmapPhotoTransformer implements Transformer<Photo, BitmapPhoto> {
  ...
  @Override public BitmapPhoto transform(Photo input) {
  ...
  private Bitmap readImage(Photo image, float scaleFactor) {
    ...
    return BitmapFactory.decodeByteArray(image.encodedImage, 0, image.encodedImage.length);
  }

  private float computeScaleFactor(Size originalSize, Size desiredSize) {
    return Math.min(originalSize.width / (float) desiredSize.width,
  ...
  private Size readImageSize(Photo image) {
    ...
    BitmapFactory.decodeByteArray(image.encodedImage, 0, image.encodedImage.length, options);
