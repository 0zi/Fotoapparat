 ...
 */
@RequiresApi(api = Build.VERSION_CODES.LOLLIPOP) public class V2AvailableLensPositionProvider
    implements AvailableLensPositionsProvider {
  ...
  private Integer getLensPositionUnsafe(String cameraId) {
    try {
      return manager.getCameraCharacteristics(cameraId).get(CameraCharacteristics.LENS_FACING);
