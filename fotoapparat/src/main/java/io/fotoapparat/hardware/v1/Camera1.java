 ...
 */
@SuppressWarnings("deprecation") public final class Camera1 implements CameraDevice {
  ...
  private void throwOnFailedToOpenCamera(LensPosition lensPosition, RuntimeException e) {
    throw new CameraException(
        "Failed to open camera with lens position: " + lensPosition + " and id: " + cameraId, e);
  }
  ...
  private void throwOnFailStartPreview(RuntimeException e) {
    throw new CameraException("Failed to start preview for camera devices: " + cameraId, e);
  }
  ...
  @Override public void setDisplayOrientation(int degrees) {
    ...
    camera.setDisplayOrientation(computeDisplayOrientation(degrees, info));
    previewStream.setFrameOrientation(imageRotation);
  }

  private int computeDisplayOrientation(int screenRotationDegrees, Camera.CameraInfo info) {
    return OrientationUtils.computeDisplayOrientation(screenRotationDegrees, info.orientation,
        info.facing == Camera.CameraInfo.CAMERA_FACING_FRONT);
  }

  private int computeImageOrientation(int screenRotationDegrees, Camera.CameraInfo info) {
    return OrientationUtils.computeImageOrientation(screenRotationDegrees, info.orientation,
        info.facing == Camera.CameraInfo.CAMERA_FACING_FRONT);
  }
  ...
  @NonNull private SwitchOnFailureParametersOperator parametersOperator() {
    ParametersOperator unsafeParametersOperator =
        new UnsafeParametersOperator(camera, parametersConverter);

    ParametersOperator fallbackOperator = new SplitParametersOperator(
        new SupressExceptionsParametersOperator(unsafeParametersOperator, logger));

    return new SwitchOnFailureParametersOperator(unsafeParametersOperator, fallbackOperator);
  }

  @Override public Capabilities getCapabilities() {
    recordMethod();

    return capabilitiesFactory.fromParameters(camera.getParameters());
  }
  ...
  @Override public Photo takePicture() {
    ...
    camera.takePicture(null, null, null, new Camera.PictureCallback() {
      @Override public void onPictureTaken(byte[] data, Camera camera) {
        photoReference.set(new Photo(data, imageRotation));
  ...
  @Override public PreviewStream getPreviewStream() {
    ...
    return isPreviewStreamInitialized() ? previewStream : PreviewStream.NULL;
  }
  ...
  @Override public RendererParameters getRendererParameters() {
    ...
    RendererParameters rendererParameters = new RendererParameters(previewSize(), imageRotation);
  ...
  @Override public FocusResult autoFocus(@NonNull FocusArea area) {
    try {
      ...
      final Rect meteringRect = area.asMeteringRect();
      final Rect focusRect = area.asFocusRect();

      final Camera.Parameters parameters = camera.getParameters();
      final List<Area> focusList = Collections.singletonList(new Area(focusRect, WEIGHT_MAX));
      final List<Area> meteringList = Collections.singletonList(new Area(meteringRect, WEIGHT_MAX));
  ...
  @NonNull private Camera.CameraInfo getCameraInfo(int id) {
