...
class CorrectTextureOrientationTask implements Runnable {
  ...
  CorrectTextureOrientationTask(TextureView textureView, int screenOrientation, float width,
      float height) {
  ...
  private static void correctRotatedDimensions(Matrix matrix, float width, float height,
      float centerHorizontal, float centerVertical) {
    float rotationCorrectionScaleHeight = height / width;
    float rotationCorrectionScaleWidth = width / height;

    matrix.postScale(rotationCorrectionScaleHeight, rotationCorrectionScaleWidth, centerHorizontal,
        centerVertical);
  }

  private static void correctRotation(Matrix matrix, int screenOrientation, float centerHorizontal,
