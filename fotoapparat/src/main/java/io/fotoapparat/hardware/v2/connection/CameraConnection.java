 ...
 */
@SuppressWarnings("MissingPermission") @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
public class CameraConnection implements ConnectionOperator {
  ...
  public CameraConnection(CameraSelector cameraSelector, CameraManager manager,
  ...
  @Override public void open(LensPosition lensPosition) {
    ...
    characteristics = new GetCharacteristicsTask(manager).execute(cameraId);

    camera = new OpenCameraTask(manager, cameraThread).execute(cameraId);
  }
