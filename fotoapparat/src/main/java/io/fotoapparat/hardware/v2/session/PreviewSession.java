 ...
 */
@RequiresApi(api = Build.VERSION_CODES.LOLLIPOP) class PreviewSession extends Session
    implements PreviewOperator {

  private final CaptureRequest captureRequest;
  private CameraCaptureSession captureSession;

  PreviewSession(CameraDevice camera, CaptureRequest captureRequest, List<Surface> surfaces,
      CameraThread cameraThread) {
    super(camera, Collections.unmodifiableList(surfaces), cameraThread);
    this.captureRequest = captureRequest;
  }

  @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP) @Override public void startPreview() {
    try {
