 ...
 */
@SuppressWarnings("NewApi") public class LensOperationsFactory {
  ...
  public LensOperationsFactory(SessionManager sessionManager,
      CaptureRequestFactory captureRequestFactory, CameraThread cameraThread) {
  ...
  public LensOperation<FocusResult> createLockFocusOperation() {
    try {
      return LensOperation.from(captureRequestFactory.createLockRequest(), handler,
          new FocusResultTransformer(), sessionManager.getCaptureSession());
  ...
  public LensOperation<ExposureResultState> createExposureGatheringOperation() {
    try {
      return LensOperation.from(captureRequestFactory.createExposureGatheringRequest(), handler,
          new ExposureResultTransformer(), sessionManager.getCaptureSession());
    } catch (CameraAccessException e) {
      throw new CameraException(e);
    }
  }
  ...
  public LensOperation<CaptureResultState> createCaptureOperation() {
    try {
      return LensOperation.from(captureRequestFactory.createCaptureRequest(), handler,
          new CaptureResultTransformer(), sessionManager.getCaptureSession());
    } catch (CameraAccessException e) {
      throw new CameraException(e);
    }
  }
