...
public class StartCameraRoutine implements Runnable {
  ...
  public StartCameraRoutine(CameraDevice cameraDevice, CameraRenderer cameraRenderer,
  ...
  private void tryToStartCamera() {
    LensPosition lensPosition =
        lensPositionSelector.select(cameraDevice.getAvailableLensPositions());

    cameraDevice.open(lensPosition);
    cameraDevice.updateParameters(initialParametersProvider.initialParameters());
    cameraDevice.setDisplayOrientation(screenOrientationProvider.getScreenRotation());
